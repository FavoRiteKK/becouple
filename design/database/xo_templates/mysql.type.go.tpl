...
func ({{ $short }} *{{ .Name }}) Delete(db XODB) error {
	...
	{{ if gt ( len .PrimaryKeyFields ) 1 }}
		// sql query with composite primary key
		const sqlstr = `UPDATE {{ $table }} SET deleted = ? WHERE {{ colnamesquery .PrimaryKeyFields " AND " }}`

		// run query
		XOLog(sqlstr, true, {{ fieldnames .PrimaryKeyFields $short }})
		_, err = db.Exec(sqlstr, true, {{ fieldnames .PrimaryKeyFields $short }})
		if err != nil {
			return err
		}
	{{- else }}
		// sql query
		const sqlstr = `UPDATE {{ $table }} SET deleted = ? WHERE {{ colname .PrimaryKey.Col }} = ?`

		// run query
		XOLog(sqlstr, true, {{ $short }}.{{ .PrimaryKey.Name }})
		_, err = db.Exec(sqlstr, true, {{ $short }}.{{ .PrimaryKey.Name }})
		if err != nil {
			return err
		}
	{{- end }}
